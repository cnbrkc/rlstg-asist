...
def _payload(reels_state, caption_state, threads_state, ses_basarili, ses_dosyasi, legacy_voice, model_reels, kullanilan_ses_modeli, model_threads, ses_modu, qa_rounds, final_video, temp_input_video, fact_state, editorial_state, duo_plan, duo_script, qa_state, qa_pass, state, input_media=None, output_media=None, mode='video'):
    """Pipeline çıktı dict'ini tek yerde üretir; video/metin modları için ortak alanları paylaşır."""
...
def _caption_calistir(router, reels_state, fact_state, editorial_state, video_state, log):
    content = girdi_birlestir(
        durumu_metne_donustur('REELS', reels_state),
        durumu_metne_donustur('FACT LOCK', fact_state),
        durumu_metne_donustur('EDITORIAL', editorial_state),
...
def _beklenen_gercek_mod(duo_plan, duo_script):
    mode = str(
        ((duo_script or {}).get("contract", {}) or {}).get("mode")
        or (duo_plan or {}).get("mode")
        or "DUO"
    ).strip().upper()
...
def _duo_ses_veya_legacy_uret(router, duo_script, legacy_text, legacy_voice, log, output_path):
    """Gerçek moda göre TTS üretir. DUO modunda legacy fallback YASAK."""
...
def _ses_sure_uyumlu_mu(ses_dosyasi, video_suresi):
    ...
    if video_suresi <= 0 or ses_suresi <= 0:
...
def _reels_ve_ses_uyumlu_uret(router, editorial_state, fact_state, video_state, notes, sure_saniye, ton, legacy_voice, log, baslangic_talimati=""):
    ...
    for deneme in range(VOICE_REGEN_MAX + 1):
        ...
        # DUO modunda legacy fallback YOK. Sadece SOLO modlarında legacy dönüşü denenir.
...
def _qa_calistir(router, video_state, fact_state, editorial_state, reels_state, caption_state, threads_state, sure_saniye, log, duo_plan=None, duo_script=None):
    content = girdi_birlestir(
        ...
        durumu_metne_donustur('EDITORIAL', editorial_state),
        durumu_metne_donustur('REELS', reels_state),
        durumu_metne_donustur('DUO PLAN', duo_plan or {}),
        durumu_metne_donustur('DUO SCRIPT', duo_script or {}),
        durumu_metne_donustur('CAPTION', caption_state),
        durumu_metne_donustur('THREADS', threads_state),
...
def _qa_regeneration_loop(router, video_state, fact_state, editorial_state, reels_state, caption_state, threads_state, duo_plan, duo_script, sure_saniye, ton, legacy_voice, log, voice_initial_instruction=''):
    ...
    try:
        ...
        log(f'⚠️ Caption üretilemedi: {str(e)[:150]}')
    ...
    try:
        ...
        log(f'⚠️ Threads üretilemedi: {str(e)[:150]}')
    ...
    for qa_round in range(MAX_QA_REGEN + 1):
        ...
        if overall == 'PASS' and not supported_targets:
            # DUO contract: PASS için gerçek mod gerçekten DUO olmalı ve ses dosyası doğrulanmış olmalı.
            if expected_mode == "DUO" and ses_modu != "DUO":
                log(f'❌ QA PASS geldi ama gerçek ses modu {ses_modu}; DUO zorunluluğu ihlali. Regeneration tetikleniyor.')
                overall = "FAIL"
                ...
                log('❌ QA PASS geldi ama DUO için doğrulanmış ses dosyası yok. Regeneration tetikleniyor.')
                overall = "FAIL"
                supported_targets = ["DUO_SCRIPT_FAIL"]
                qa_state["overall"] = "FAIL"
                qa_state["regeneration_targets"] = supported_targets
        ...
        if creative_needed:
            ...
            if 'COVER_FAIL' in target_set:
                instruction += 'Kapak/hook uyumunu düzelt. '
            if 'VOICEOVER_FAIL' in target_set:
                instruction += 'Seslendirmeyi doğal, kısa ve hedef kelime aralığında tut. '
            reels_state, model_reels, duo_plan, duo_script, ses_basarili, kullanilan_ses_modeli, ses_modu, ses_dosyasi = _reels_ve_ses_uyumlu_uret(
        ...
        if downstream_caption:
            try:
                ...
                log(f'⚠️ Caption regeneration başarısız: {str(e)[:150]}')
        ...
        if downstream_threads:
            try:
                ...
                log(f'⚠️ Threads regeneration başarısız: {str(e)[:150]}')
...
def pipeline_calistir(router, video_bytes, mime_type, temp_input_video, video_analiz_notlari, metin_uretim_notlari, sure_saniye, icerik_tonu, secilen_ses_ingilizce, log_ekle, ilerlemeyi_guncelle=None):
    ...
    video_state, _ = _forensic_analiz_calistir(router, video_bytes, mime_type, video_analiz_notlari, sure_saniye, log_ekle)
    ...
    state['editorial_state'] = editorial_state

    _ilerleme(ilerlemeyi_guncelle, 4)
    log_ekle('🎙️ Reels hazırlanıyor (Cover + Hook + Voiceover + Duo)...')
    ...
    state['reels_state'] = reels_state
    state['duo_plan'] = duo_plan
    state['duo_script'] = duo_script
    state['ses_modu'] = ses_modu
    state['qa_regeneration_rounds'] = qa_rounds
    state['qa_pass'] = qa_pass
    ...
    if not qa_pass:
        ...
        log_ekle('❌ QA PASS alınamadı; TTS/render aşaması güvenli biçimde durduruldu.')
        return _payload(
            reels_state, caption_state, threads_state, False, '', legacy_voice,
    ...
    log_ekle('🎬 Videoya AI sesi ekleniyor (FFmpeg)...')
    ...
    return _payload(
        reels_state, caption_state, threads_state, ses_basarili, ses_dosyasi,
...
def metin_pipeline_calistir(router, metin, icerik_tonu, secilen_ses_ingilizce, log_ekle, ilerlemeyi_guncelle=None, sure_saniye=30):
    ...
    state = {}
    video_state = {
        'video_identity': {'brand': 'UNKNOWN', 'exact_model': 'UNKNOWN', 'confidence': 'unknown', 'source': 'telegram_text'},
        'observed_facts': [metin],
        'unknowns': [],
        'possible_inference': [],
        'viral_arastirma_ihtiyaclari': ['Metindeki araç/konu kimliğini ve güncel iddiaları doğrula.'],
        'visual_opportunities': ['Metin tabanlı üretim; video görsel zaman çizelgesi yok.'],
        'timeline': []
    }
    state['video_state'] = video_state
    ...
    editorial_state, _ = _editorial_calistir(router, video_state, fact_state, metin, log_ekle)
    state['editorial_state'] = editorial_state
    ...
    state['qa_regeneration_rounds'] = qa_rounds
    state['qa_pass'] = qa_pass
    state['caption_state'] = _caption_state_normalize(caption_state)
    state['threads_state'] = _threads_state_normalize(threads_state)
    state['qa_state_final'] = qa_state
